...
from database import Course, Group, get_session, GroupsCourses, CoursesLessons
from services.course_service import CourseService
from api.deps import get_user_group, get_group_by_id_with_courses, get_group_course_with_courses, \
    get_course, get_group_course, get_course_lesson
...
# я зачем зарефакторил тестовый файл? :DD
# пусть будет
...
async def get_courses(group_course: GroupsCourses = Depends(get_group_course_with_courses)) -> CourseResponse:
...
async def delete_course(course: Course = Depends(get_course),
                        session: AsyncSession = Depends(get_session)):
