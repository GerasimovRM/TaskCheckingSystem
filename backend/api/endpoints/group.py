...
async def get_user_groups(current_user: User = Depends(get_current_active_user),
    ...
    user_groups = await UsersGroupsService.get_user_groups(current_user.id, session)
...
async def put_group(req: GroupDto,
                    current_user: User = Depends(get_current_active_user),
                    session: AsyncSession = Depends(get_session)) -> GroupDto:
    # check group access
    user_group = await UsersGroupsService.get_user_group(current_user.id,
    ...
    group = await GroupService.get_group_by_id(req.id, session)
    role = user_group.role
    current_user_is_admin = await UserService.is_admin(current_user.id, session)

    if not current_user_is_admin and role != UserGroupRole.OWNER:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Not enough rights to edit this group")

    group.update_by_pydantic(req)
...
async def post_group(group_name: str,
    ...
    if not current_user_is_admin_or_teacher:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Not enough rights to create group")
