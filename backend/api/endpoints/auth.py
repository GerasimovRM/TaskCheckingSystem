...
from models.site.auth import AuthLogin
...
async def login_by_login_password(auth_login: AuthLogin,
                                  response: Response,
                                  session: AsyncSession = Depends(get_session),
                                  refresh_token: Optional[str] = Cookie(None)
                                  ) -> TokenWithUserData:
    try:
        db_user = await authenticate_user(auth_login.login, auth_login.password, session)
...
@router.post("/login_vk", response_model=TokenWithUserData)
async def login_vk(response: Response,
                   vk_code: str,
                   session: AsyncSession = Depends(get_session),
                   refresh_token: Optional[str] = Cookie(None)):
    async with aiohttp.ClientSession() as http_session:
        ...
        logging.debug(
            f"https://oauth.vk.com/access_token?{'&'.join(map(lambda x: f'{x[0]}={x[1]}', data.items()))}")
        async with http_session.get("https://oauth.vk.com/access_token",
                                    params=data) as response_session:
...
async def refresh(response: Response,
    ...
    if db_refresh_token:
        ...
        jwt_access_token = await create_access_token_user(db_user, session)
        jwt_refresh_token = await create_refresh_token_user(db_user, session, refresh_token)
        response.set_cookie("refresh_token", jwt_refresh_token, httponly=True, samesite="none",
                            secure=True)
        return TokenWithUserData(access_token=jwt_access_token, user=UserDto.from_orm(db_user))
    else:
        raise HTTPException(
